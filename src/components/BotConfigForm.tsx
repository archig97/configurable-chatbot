...
export default function BotConfigForm({ botConfig, setBotConfig, isMock }: Props) {
    ...
    <div className="rounded-2xl shadow-lg border border-gray-400 p-6 bg-red-400 space-y-4 transition-colors duration-200">
      ...
      <label className="block">
        ...
        <input className="w-full rounded-lg border border-gray-300 px-3 py-2 text-black bg-white dark:text-white dark:bg-gray-700 focus:outline-none focus:ring-2 focus:ring-indigo-400" value={localConfig.name} onChange={(e) => setLocalConfig((c) => ({ ...c, name: e.target.value }))} />
      </label>
      <label className="block">
        <span className="block text-sm text-gray-700 mb-1">Persona</span>
        <textarea className="w-full rounded-lg border border-gray-300 text-black bg-white dark:text-white dark:bg-gray-700 px-3 py-2 min-h-[80px] focus:outline-none focus:ring-2 focus:ring-indigo-400" value={localConfig.persona} onChange={(e) => setLocalConfig((c) => ({ ...c, persona: e.target.value }))} />
      </label>
      <label className="block">
