    ...
    Clear Chat
      ...
      {m.role === "user" ? "You" : "Bot"}
        ...
        <div className="mt-3 flex gap-2 sticky bottom-0 bg-white pt-2">
          <input className="flex-1 rounded-xl border border-gray-300 px-3 py-2 text-black bg-white dark:text-white dark:bg-gray-700 focus:outline-none focus:ring focus:ring-indigo-200" placeholder="Type your message…" value={input} onChange={(e) => setInput(e.target.value)} onKeyDown={(e) => { if (e.key === "Enter" && !e.shiftKey) { e.preventDefault(); handleSendClick(); } }} disabled={isSending} />
